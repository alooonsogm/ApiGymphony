...
namespace ApiGymphony.Repositories
{
    public class RepositoryGymphony
    {
        ...
        public async Task<List<DatosHoraPico>> GetHorasPicoAsync()
        {
            ...
            var consulta = from registro in this.context.RegistroAforo
                           where
                           registro.HoraEntrada >= fechaLimite &&
                           registro.HoraEntrada.Hour >= 9 && registro.HoraEntrada.Hour <= 21
                           group registro by registro.HoraEntrada.Hour into grupoHoras
                           orderby grupoHoras.Key ascending
        ...
        public async Task<List<DatosEvolucion>> GetEvolucionSociosAsync()
        {
            ...
            for ( int i = 11 ; i >= 0 ; i-- )
            {
                ...
                var consultaBajas = from a in afiliaciones
                                    where a.FechaBaja != null && a.FechaBaja.Value.Year == year && a.FechaBaja.Value.Month == month
                                    select a;
